...
{ 
  ...
  // std::cout << " === CREATED, thread id = " << threadId << std::endl;
    ...
    for (int i = p->numVisited; i < numCities; i++) 
    {
      ...
      if (p1->numVisited == numCities) // If we visited them all
      {
	      ...
	      if (p1->length < shortestPath->length) {
            ...
            *shortestPath = *p1;
            // std::cout << "changing the shorted path: "; p1->Print();
        }
  ...
  // std::cout << " EXIT, thread id = " << threadId << " Path: ";
  // shortestPath->Print();
  return NULL;
}
...
int main(int argc, char *argv[])
{
  ...
  // std::cout << "======== Printing all the shortes paths =========" << std::endl;
  // for (auto itr = shortestPaths.begin(); itr != shortestPaths.end(); itr++) {
  //   itr->Print();
  // }
  
  // auto minPath = std::min_element( shortestPaths.begin(), shortestPaths.end(),
